...
int main(int argc, char const* argv[]) {
  ...
  for (size_t i = 0; i < 78; i++) {
    ...
    ASSERT_EQUAL(a, b,
                 "Array must return a number corresponding to its count.");
    checkIntegrity(&test);
    unsigned int last = test.Last();
    unsigned int popLast = test.PopLast();
    ASSERT_EQUAL(last, popLast,
                 "Array must return the last number when being PopLast.");
    checkIntegrity(&test);
  }

  ASSERT_EQUAL(
      test.Count(), 0,
      "Array must have a count of zero after removing every elements.");
  checkIntegrity(&test);
